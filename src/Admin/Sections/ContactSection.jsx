...
import { FaTrash, FaEnvelope, FaEnvelopeOpen, FaCheck } from 'react-icons/fa';
...
const ContactSection = () => {
  ...
  const handleMarkAllAsRead = () => {
    try {
      ...
      setMessages(updatedMessages);
      localStorage.setItem(ENTITIES.MESSAGES, JSON.stringify(updatedMessages));
      showStatusMessage('All messages marked as read');
    } catch (error) {
      showStatusMessage('Error updating messages: ' + error.message, 'error');
    }
  };
        ...
        <div className="flex space-x-2">
          ...
          {messages.length > 0 && (
            <button
              ...
              className="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded flex items-center text-sm"
            >
